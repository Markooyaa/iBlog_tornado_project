  ...
  return (
    <div className={showSidebar ? "fixed bottom-0 top-0 left-0" : ""}>
      {showSidebar && (
        <div
          onClick={() => setShowSidebar(false)}
          className="w-full h-full opacity-80 bg-slate-800 z-10 fixed top-0 left-0 sm:w-0 flex"></div>
      )}
      <section
        className={
          showSidebar
            ? "flex"
            : "hidden sm:inline-flex  flex-col items-start gap-[10px] px-[49px] py-[40px]"
        }>
        <div
          className={
            showSidebar
              ? "absolute flex flex-col gap-[45px] bg-white z-20 sm:pt-[40px] px-[20px] sm:w-0 h-full"
              : "flex flex-col gap-[45px] w-[250px]"
          }>
          <Logo></Logo>
          <button
          ...
          <div
            className={
              showSidebar
                ? "flex"
                : "hidden w-[245px] h-[42px] px-[15px] items-center gap-[5px]"
            }>
    ...
    <Link to={url}>
      <div
        className={
          showSidebar
            ? "flex"
            : "hidden sm:flex h-[46px] items-center flex-grow"
        }>
...
function SidebarLink(props) {
  return (
    <Link
      to={props.url}
      className="capitalizetext-[14px] px-[16px] py-[8px] hover:bg-orange-500 hover:text-white">
      {props.title}
    </Link>
  );
}
